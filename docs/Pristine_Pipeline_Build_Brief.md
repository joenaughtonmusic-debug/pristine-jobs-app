...
1. **One lead record — LOCKED.** Today leads split across `sales_leads` (website form), `admin_enquiries` (manual/phone), and inbound email in `communications`.
   ...
   - **Per-source starting column:** high-volume channels **Bark and Buildercrack enter at `contacted`**, not `new`, so they don't flood the top of the board. All other channels start at `new`. Intake sets the starting status from `source`.
...
*App changes:* make `/admin/enquiries` create/write `sales_leads` rows (or migrate `admin_enquiries` into it); ensure `/api/public/sales-leads` is the single intake endpoint; attach inbound `communications` to a lead via `sales_lead_id`; fire the new-lead notification on every path. Render the pipeline as a Kanban board (columns = statuses; card = name · suburb · service). **Intake sets the starting status from `source`: `bark` and `buildercrack` → `contacted`; everything else → `new`.**

*Paste-prompt:*
> Read `docs/PIPELINE_BUILD_BRIEF.md`. Implement Phase 1 only: consolidate lead intake so `sales_leads` is the single pipeline record. Route manual enquiries (`admin_enquiries`) and inbound-email leads into `sales_leads`, add `communications.sales_lead_id`, and ensure `sendLeadNotificationToJoe` fires on every intake path exactly once. Set the starting status from `source` — `bark`/`buildercrack` start at `contacted`, all others at `new`. Render the pipeline as a Kanban board of name·suburb·service cards across the status columns. Keep `/api/public/sales-leads` as the one public intake endpoint. Follow Section 3 and verify at runtime.
